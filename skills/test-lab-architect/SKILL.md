...
description: "Arquiteta laboratórios de teste pra apps com IA no meio. Discovery do caso, força check de gabarito (ground truth), decide lab inline/standalone/híbrido, granularidade (binária, LLM-as-judge, híbrida), isolamento de orquestradores externos (n8n, edge functions), modelo de reset, promoção lab→prod, UX do operador. Output: markdown estruturado pra Lovable/Cursor/manual. Use em: lab de testes, validation lab, ambiente de testes pra IA, 'como vou validar a IA?', 'medir assertividade', 'testar prompts antes de prod', 'comparar versão de prompt', human-in-the-loop, ground truth design, AI assertiveness. NÃO use pra: gerar código (lovable-router), executar evals em traces, revisar PRD (product-discovery-prd)."
---
