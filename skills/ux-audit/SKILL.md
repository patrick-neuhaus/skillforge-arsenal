---
name: ux-audit
description: "Audita, revisa, analisa, avalia e melhora UX em apps web/mobile. Identifica problemas de usabilidade, valida WCAG 2.2, verifica dark patterns, performance percebida. Gera recomendações com severidade. Use SEMPRE em: UX, usabilidade, audit, auditoria, 'revisa a UX', 'olha a interface', 'feedback de UX', 'melhorar a experiência', 'usuário tá perdido', 'tá difícil de usar', heurísticas, Nielsen, 'fluxo tá ruim', evaluate usability, assess accessibility, redesign flow, validate WCAG, analyze user flow. Não confundir com ui-design-system (cria/padroniza componentes) nem trident --design (review de código frontend). Foco: EXPERIÊNCIA, não código nem design system."
---
