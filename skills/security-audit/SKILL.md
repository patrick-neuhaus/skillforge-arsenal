...
Pipeline de 3 agentes: **Scanner → Verifier → Arbiter**. Dois modos: Code (OWASP Top 10 2025 + Supabase RLS + secrets + XSS + auth) e VPS (hardening + portas + containers + firewall). Camada determinística (grep/SAST) roda antes dos agentes.

## Options

| Option | Description | Default |
|--------|-------------|---------|
| `--mode <m>` | Modo: `code`, `vps`, `combined` | auto-detect |
| `--target <t>` | Repo path, SSH host, ou GitHub URL | current dir |
...
- [ ] Phase 0: Detect Mode ⚠️ REQUIRED
  - [ ] 0.1 Identificar modo (Code/VPS/Combined)
  - [ ] 0.2 Validar acesso (repo clonado / SSH disponível)
  - [ ] 0.3 Se Combined: Code primeiro, VPS depois
- [ ] Phase 1: Deterministic Layer (Code only) ⚠️ REQUIRED
  - [ ] 1.1 Secrets scan (grep patterns)
  - [ ] 1.2 npm audit / dependency check
  - [ ] 1.3 XSS patterns (dangerouslySetInnerHTML, href)
  - [ ] 1.4 RLS verification (migrations)
  - [ ] 1.5 .env no git check
  - [ ] 1.6 SAST tools se disponíveis (Semgrep/CodeQL)
...
| "auditoria completa", "tudo" | Combined (Code primeiro) |

**Se ambíguo:** pergunte. Não assuma.

## Phase 1: Deterministic Layer (Modo Code)
...
Verificações: secrets hardcoded, npm audit, XSS patterns, .env no git, RLS em migrations, SAST tools (Semgrep/CodeQL se disponíveis).
...
- [ ] Camada determinística rodou (Modo Code)?
